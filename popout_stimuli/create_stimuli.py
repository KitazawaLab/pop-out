...
class CreateStimuli:
    def __init__(self, width, height, base):
        ...
        self.w = width
        if base == "white":
            self.base = 255*np.ones((height, width, 3), dtype=np.uint8)
        elif base == "black":
            self.base = np.zeros((height, width, 3), dtype=np.uint8)
        elif base == "noise":
            self.base = np.random.randint(0, 256, size=(height, width, 3), dtype=np.uint8)
    ...
    def color_rotate_noise_5_5(self, r_w, r_h, out_dir, c1, c2):
        '''
            ...
            if image==c1:
                ...
                for i in range(popout_list.shape[0]):
                    ...
                    if int(popout_list[i]) == 0:
                        self.rotated_rectangle(base_image, rotated_rect, color_dic[c2])
                    else:
                        self.rotated_rectangle(base_image, rotated_rect, color_dic[c1])
    ...
    def color_rotate_noise_6_6(self, r_w, r_h, out_dir, c1, c2):
        '''
    ...
    def random_color_rotated_noise(self, r_w, r_h, object_num, out_dir, c1, c2):
        '''
        ...
        while obj_num <= object_num:
            ...
            if obj_num == 0:
                ...
                x = np.random.randint(diagonal, self.w - diagonal)
                y = np.random.randint(diagonal, self.h - diagonal)
                posision = (x, y)
                ...
                rotated_rect = (posision, (r_w, r_h), angle)
                if popout_list[obj_num] == 0:
                    self.rotated_rectangle(current_image, rotated_rect, color_dic[c1])
                else:
                    self.rotated_rectangle(current_image, rotated_rect, color_dic[c2])
